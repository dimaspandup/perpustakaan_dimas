...
<table class="table table-hover table-striped">
	<thead>
		<th>NO</th><th>Tanggal Peminjaman</th><th>Tanggal harus kembali</th><th>Nama Buku</th><th>Status</th><th>Aksi</th>
	</thead>
	<tbody>
		<?php  
		include "koneksi.php";
		...
		while($dt_histori=mysqli_fetch_array($qry_histori)){
			...
			$buku_dipinjam.="</ol>";
			// menampilkan status sudah kembali atau belum kembali
			...
			if (mysqli_num_rows($qry_cek_kembali)>0) {
				...
				$button_kembali="<a href='pengembalian.php?id=".$dt_histori['id_peminjaman_buku']."' class='btn btn-warning' onclick='return confirm(\"Yakin ingin mengembalikan?\")'>Kembalikan</a>";
			}
		?>
			<tr>
